...
function get20s(array){
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    if (str.charAt(1) === '9'){
      the20s[i] = array[i].name;
      console.log(the20s[i]);
    }
  }
...
function removeArtist(array, index) {
  ...
  console.log('Items remaining: ' + array.length);
...
function addArtist(artist, array){
...
function lotsOfArt(array){
  ...
  for (let i = 0; i < array.length; i++){
    if (array[i].paintings > 100){
      overAchievers[i] = array[i].name;
      num++;
    }
  }

  console.log('Number of artists with > 100 paintings: ' + num + '\n');

  for (let i = 0; i < overAchievers.length; i++){
    console.log(overAchievers[i]);
  }
